...
**Подробный банк вопросов и чек-листы валидации ответов:** `references/clarifying-questions.md`. Используй его как справочник по категориям (проблема, пользователи, флоу, AC, edge cases, out of scope, зависимости, нон-функционалка) и анти-паттернам ответов заказчика.
...
ВАЖНО!!! ВСЕГДА Сохраняй спеку в корне проекта в:
- `specs/feature-name/` — если такая директория существует
  Если директории **specs** нет, создай ее сам.
...
- `references/clarifying-questions.md` — банк уточняющих вопросов и чек-листы валидации ответов заказчика (для интервью на Шаге 1)
