...
1. **параллельно/батчем, если возможно**:
   ...
   Эти два агента работают на разных файлах (`documentation-writer` — KDoc в production, `unit-tester` — `commonTest/`), не конфликтуют, поэтому батчатся параллельно. **Не запускать последовательно** — это удваивает время.
...
2. **Последовательно после батча** (каждый зависит от результатов предыдущих):
   ...
   - `qa-bug-hunter` — поиск крэшей, ANR, edge-кейсов.
...
**Каждому subagent передавать самодостаточный prompt** — список файлов, контекст фичи, ссылки на эталоны в репо. Subagent не видит основной диалог. Просить отчитаться кратко (файлы, кейсы, статус), без копий кода — иначе раздувается основной контекст.